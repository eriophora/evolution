...
for exp, iters, const, stats in experiments:
    ...
    for i in range(iters):
        w.iterate()
        if not i:
            keys = w.agents[0].genome.gene_map.keys()
        str1 = ''
        for k in keys:
            str1+=str(w.agents[0].genome.gene_map[k])
        for k in keys:
            str1+=str(w.agents[1].genome.gene_map[k])
        genomes.append(str1)
    # delete above after
